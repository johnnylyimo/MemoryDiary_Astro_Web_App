...
export const CreateDisplayMemory = () => {
        ...
        <main className="w-full h-auto flex flex-col lg:flex-row gap-8 text-center p-4">
            <section className="max-w-screen-sm mx-auto lg:max-w-xxl rounded-lg p-2 bg-[#ce93d8]">
                ...
                <form  className="p-2 mt-6"  onSubmit={onSubmitMemory}>
                    ...
                    <textarea rows={4}  value={editMemoryPopUp ? '' : inputedMemory.content || ""}
  className="block w-full text-base p-2 text-gray-900 bg-[#ce93d8] rounded-lg border-4 border-[#ac5eb9dc] focus:outline-none" placeholder='Write new memory here ...' disabled={editMemoryPopUp ? true:false}  onChange={({target})=> setInputedMemory({...inputedMemory, content: target.value})}>
            ...
            {/* Edit Popup */}
            {
            editMemoryPopUp ? <div className="absolute top-[65%] md:top-[35%] left-[50%] translate-x-[-50%] translate-y-[-50%] bg-[#E1BEE7] w-[90%] md:w-[45%] flex justify-center items-center flex-col rounded-lg shadow-xl h-auto p-2 mx-auto">
                ...
                <div className="w-full flex justify-around">
                    <button className="my-5 w-auto px-8 h-10 bg-red-600 font-semibold text-white rounded-md shadow-md hover:shadow-lg" onClick={onDeleteMemory}>
                        Delete
                    </button>
                    <button className="my-5 w-auto px-8 h-10 bg-blue-600 text-white font-semibold rounded-md shadow-md hover:shadow-lg" onClick={onUpdateMemory}>Update
                    </button>
